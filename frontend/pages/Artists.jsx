...
const ArtistProfileContainer = styled(Box)`
  ...
  align-items: center;
  margin-left: ${(props) => (props.$isMobile ? "calc(65% - 50vw)" : "calc(91% - 50vw)")};
`;
...
export default function ArtistPage() {
  ...
  const [allSongsLocal, setAllSongsLocal] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchArtistData = async () => {
      try {
        ...
        const timestampedImage = artistData.image?.trim()
          ? `${artistData.image}${artistData.image.includes("?") ? "&" : "?"}timestamp=${Date.now()}`
          : null;

        setArtist({ ...artistData, image: timestampedImage });
        setTopSongs(artistRes.data.topSongs || []);
        setAllSongsLocal(artistRes.data.allSongs || []);
  ...
  if (loading) {
    return (
      <Container sx={{ py: 4 }}>
        <Typography color="white">Loading artist...</Typography>
      </Container>
    );
  }
  ...
  if (!artist) {
    return (
      <Container sx={{ py: 4 }}>
        <Typography color="white">Artist not found.</Typography>
      </Container>
    );
  }
    ...
    <Container sx={{ paddingBottom: "150px" }}>
      <ArtistHeader>
        <ArtistProfileContainer $isMobile={isMobile}>
          <ArtistBanner $banner={bannerImage} />
          <ArtistImageWrapper>
            {artist.image ? (
      ...
      <List>
        {topSongs.map((song) => (
          <ListItem
            key={song._id}
            button
            onClick={() => playTrack(song, topSongs, { type: "artist-top-songs", artistSlug: slug })}
      ...
      <List>
        {(showFullDiscography ? allSongsLocal : allSongsLocal.slice(0, 5)).map((track) => (
          <ListItem
            key={track._id}
            button
            onClick={() => playTrack(track, allSongsLocal, { type: "artist-discography", artistSlug: slug })}
            sx={{ transition: "0.3s", "&:hover": { backgroundColor: "#333" } }}
          >
            <ListItemText
              primary={track.title}
              secondary={new Date(track.releaseDate).toLocaleDateString()}
            />
          </ListItem>
        ))}
      </List>
      ...
      {allSongsLocal.length > 5 && (
        <ExpandButton
          variant="outlined"
          onClick={() => setShowFullDiscography(!showFullDiscography)}
        >
